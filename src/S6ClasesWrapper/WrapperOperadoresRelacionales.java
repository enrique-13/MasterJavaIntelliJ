package S6ClasesWrapper;

public class WrapperOperadoresRelacionales {
    public static void main(String[] args) {

        Integer num1 = Integer.valueOf(1000);
        Integer num2 = num1;

        System.out.println("num1 = " + num1);
        System.out.println("num2 = " + num2);

        System.out.println("Son el mismo objeto? " + (num1 == num2));


    //-- Ejemplo n°2
        num2 = 1000;
        System.out.println("num1 = " + num1);
        System.out.println("num2 = " + num2);
    //- COMPARA LA INSTANCIA - objeto
        System.out.println("Son el mismo objeto? " + (num1 == num2));
    //- COMPARA EL VALOR
        System.out.println("tienen el mismo valor? " + (num1.equals(num2)));
    //- intValue() CONVIERTE A TIPO PRIMITIVO
        System.out.println("tienen el mismo valor? " + (num1.intValue() == num2.intValue()));


    //-- Ejemplo n°3
        num2 = 500;
    //-- hace un autoAndBoxin-"desmpaquetar" - invoca a .intValue() para
        boolean condicion = num1 > num2;
        System.out.println("condicion = " + condicion);
        //convertir de obj referencia a un primitivo
        boolean condicion2 = num1.intValue() < num2.intValue();
        System.out.println("condicion2 = " + condicion2);
    }
}
